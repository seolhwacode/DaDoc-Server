...
<div class="container">
	...
	<form name="myForm" action="${pageContext.request.contextPath}/challenge/private/update.do" method="post" enctype="multipart/form-data" id="myForm" >
		...
		<div class="form-check">
			...
			<input class="form-check-input" type="radio" id="normal" name="type" value="normal" selected />
		</div>
		<!-- id -->
		<div>
		...
		<div class="input-group mb-3">
	 		 ...
	 		 <input type="text" class="form-control" id="title" name="title" aria-describedby="basic-addon3" value="${dto.title }">
		</div>
		<!-- category radio -->
		<div class="category">
		...
		<div class="form-check">
			...
			<input class="form-check-input" type="radio" id="reading" name="category" value="reading" ${dto.category eq 'reading' ? 'selected' : '' }/>
		</div>
		<div  class="form-check">
			<label class="form-check-label" for="writing">Writing</label>
			<input class="form-check-input" type="radio" id="writing" name="category" value="writing" ${dto.category eq 'writing' ? 'selected' : '' } />
		</div>
		<div  class="form-check"> 
			<label class="form-check-label" for="listening">Listening</label>
			<input class="form-check-input" type="radio" id="listening" name="category" value="listening" ${dto.category eq 'listening' ? 'selected' : '' } />
		</div>
		</div>
		...
		<div class="input-group">
		  ...
		  <textarea class="form-control"  type="text" id="description" name="description" aria-label="With textarea" value="${dto.description }"></textarea>
		</div>
		...
		<div class="period">
			...
			<select  class="form-select  form-select-sm" aria-label=".form-select-sm example" name="period" id="period">
				...
				<option value="2" ${dto.period eq '1' ? 'selected' : '' }>2</option>
				<option value="3" ${dto.period eq '1' ? 'selected' : '' }>3</option>
				<option value="4" ${dto.period eq '1' ? 'selected' : '' }>4</option>
				<option value="5" ${dto.period eq '1' ? 'selected' : '' }>5</option>
				<option value="6" ${dto.period eq '1' ? 'selected' : '' }>6</option>
				<option value="7" ${dto.period eq '1' ? 'selected' : '' }>7</option>
			</select>
		</div>	
		<!-- 이미지 업로드 -->
		<div>
